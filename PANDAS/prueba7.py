...
ventana.title('Productos Casio con un precio de $249 y 101 de dimension ')
...
def datos_excel():
	...
	try:
		...
		df=pd.read_excel("camaras2.xlsx",sheet_name="csv_camaras_2.csv",header=0, usecols=[0,1,2,3,4,5,6,7])
	...
	for fila in df_fila:
		#Lo que hace es buscar los datos que empiecen por una S
            palabra_inicial=fila[0].startswith('Sony')
		
            if(palabra_inicial==True and fila[3]<8 and fila[7]==149):
